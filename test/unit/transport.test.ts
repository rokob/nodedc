...
test('getTransportInfo emits RFC 9842-compatible fixed headers', () => {
  ...
  assert.deepEqual([...zstdInfo.headerBytes.subarray(0, 8)], [0x5e, 0x2a, 0x4d, 0x18, 0x20, 0x00, 0x00, 0x00]);
...
test('PreparedDictionary one-shot transport framing round-trips for zstd and brotli', async () => {
  const zstd = new PreparedDictionary({ algorithm: 'zstd', bytes: Buffer.from('zstd transport dict') });
  const brotli = new PreparedDictionary({ algorithm: 'brotli', bytes: Buffer.from('brotli transport dict') });
  ...
  assert.deepEqual(await brotli.decompress(brotliCompressed, { transport: 'transport' }), brotliInput);
});
...
test('transport decompression rejects mismatched dictionary headers', async () => {
  const first = new PreparedDictionary({ algorithm: 'zstd', bytes: Buffer.from('first transport dict') });
  const second = new PreparedDictionary({ algorithm: 'zstd', bytes: Buffer.from('second transport dict') });
  const input = Buffer.from('first transport dict :: body');

  const compressed = await first.compress(input, { transport: 'transport' });

  await assert.rejects(() => second.decompress(compressed, { transport: 'transport' }), /Invalid dcz transport header/);
});
