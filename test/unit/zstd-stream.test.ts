...
test('PreparedDictionary zstd supports streaming compress and decompress', async () => {
  ...
  const compressedPromise = collect(
    Readable.from([input.subarray(0, 18), input.subarray(18)]).pipe(compressedStream),
  );
...
test('PreparedDictionary zstd supports multiple compressor streams from one dictionary', async () => {
  ...
  const firstCompressed = await collect(
    Readable.from([firstInput]).pipe(dictionary.createCompressStream({ quality: 3 })),
  );
...
test('PreparedDictionary zstd supports interleaved writes across multiple compressor streams', async () => {
  const dictionary = new PreparedDictionary({
    algorithm: 'zstd',
    bytes: Buffer.from('streaming zstd dictionary for interleaved multi-stream writes'),
  });
  ...
  const [firstCompressed, secondCompressed, thirdCompressed] = await Promise.all([
  ...
  const [firstRoundTrip, secondRoundTrip, thirdRoundTrip] = await Promise.all([
    collect(
      Readable.from([firstCompressed.subarray(0, 9), firstCompressed.subarray(9)]).pipe(
        dictionary.createDecompressStream(),
      ),
    ),
    collect(
      Readable.from([secondCompressed.subarray(0, 7), secondCompressed.subarray(7)]).pipe(
        dictionary.createDecompressStream(),
      ),
    ),
    collect(
      Readable.from([thirdCompressed.subarray(0, 11), thirdCompressed.subarray(11)]).pipe(
        dictionary.createDecompressStream(),
      ),
    ),
  ]);
